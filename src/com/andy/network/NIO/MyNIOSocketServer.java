...
public class MyNIOSocketServer {
    ...
    public void start() {
        ...
        try (ServerSocketChannel serverSocketChannel = ServerSocketChannel.open()) {
            //Socket：ip+port
            serverSocketChannel.socket().bind(new InetSocketAddress(8011));
            //设置为非阻塞模式
            ...
            //创建消息处理器
            ServerHandlerBs handler = new ServerHandlerBs(1024);
    ...
    public static void main(String[] args) {
        MyNIOSocketServer server = new MyNIOSocketServer();
        new Thread(() -> {
            try {
